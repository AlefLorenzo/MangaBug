...
export const getProfile = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            'SELECT id, username, avatar_url, xp, level, title, last_active_at, created_at, is_public FROM users WHERE id = ?',
            [id]
        );
        ...
        const [history] = await pool.query('SELECT COUNT(*) as count FROM reading_history WHERE user_id = ?', [id]);
        const [favorites] = await pool.query('SELECT COUNT(*) as count FROM favorites WHERE user_id = ?', [id]);
        ...
        // Get last 5 works read
        const [recentHistory] = await pool.query(`
            SELECT 
                w.id as work_id, 
                w.title, 
                w.cover_url, 
                rh.progress_percentage as progress,
                rh.last_read
        ...
        res.json({
            ...
            stats: {
                total_chapters: history[0].count || 0,
                total_favorites: favorites[0].count || 0
            },
...
export const updateAvatar = async (req, res) => {
    ...
    try {
        ...
        // Use sharp for production processing
...
export const updateBasicInfo = async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            'UPDATE users SET username = ?, email = ?, is_public = ? WHERE id = ?',
            [username, email, is_public === undefined ? 1 : is_public, id]
        );

        const [rows] = await pool.query(
            'SELECT id, username, email, avatar_url, xp, level, title, is_public, created_at FROM users WHERE id = ?',
            [id]
        );
...
export const getFavorites = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            'SELECT w.* FROM works w JOIN favorites f ON w.id = f.work_id WHERE f.user_id = ?',
            [id]
        );
        res.json(rows);
    } catch (error) {
...
export const getHistory = async (req, res) => {
    ...
    try {
        ...
        if (uniqueWorks === 'true') {
            query += ` AND rh.id IN (
                SELECT MAX(id) FROM reading_history WHERE user_id = ? GROUP BY work_id
            )`;
        }

        query += ` ORDER BY rh.last_read DESC`;

        const params = uniqueWorks === 'true' ? [userId, userId] : [userId];
...
export const toggleFavorite = async (req, res) => {
    ...
    try {
        const [exists] = await pool.query('SELECT * FROM favorites WHERE user_id = ? AND work_id = ?', [userId, workId]);
        if (exists.length > 0) {
            await pool.query('DELETE FROM favorites WHERE user_id = ? AND work_id = ?', [userId, workId]);
            res.json({ success: true, message: 'Removido dos favoritos', action: 'removed' });
        } else {
            await pool.query('INSERT INTO favorites (user_id, work_id) VALUES (?, ?)', [userId, workId]);
...
export const getCurrentUser = async (req, res) => {
    ...
    try {
        // Update last active
        await pool.query('UPDATE users SET last_active_at = CURRENT_TIMESTAMP WHERE id = ?', [id]);

        const [rows] = await pool.query(
            'SELECT id, username, email, avatar_url, xp, level, title, is_public, is_admin, created_at FROM users WHERE id = ?',
            [id]
        );
...
export const updatePreference = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        await pool.query('UPDATE users SET clean_mode = ? WHERE id = ?', [clean_mode ? 1 : 0, userId]);
...
export const addHistory = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        const [chapters] = await pool.query('SELECT work_id FROM chapters WHERE id = ?', [chapterId]);
        ...
        // Reward XP for reading based on level rules
        try {
...
export const deleteHistory = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        await pool.query('DELETE FROM reading_history WHERE work_id = ? AND user_id = ?', [workId, userId]);
